...
У вас должен быть установлен Rust Toolchain (Rust и Cargo, версия 1.75+). 
...
Собранные бинарные файлы и компонентны библиотеки будут расположены в папке `target\release\`.

В режиме разработки можно запускать отдельные бинарные крейты погружаюсь в их каталог. 
Например, `cli_comparer`. Исходим, что мы всё ещё находимся в корне проекта.
...
С помощью методов `Parser` мы обеспечиваем выгрузку данных из файлов нужных форматов,
а затем формируем векторы с набором `YPBankTransaction`.

В текущей версии сравнивается только совпадение экземпляров структур. Вот результат,
...
Все методы трёх крейтов документированы. Это можно использовать для сборки документации,
